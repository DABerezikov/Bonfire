...
Выводи ответы **краткими** в формате diff. Объясняй WHAT (суть в одно предложение), потом показывай изменения. Без лишних деталей и пояснений.

- Отвечай кратко. Без вступлений и заключений.
- Для кода: только изменения, не переписывай весь файл.
- Если ответ очевиден — просто покажи результат.
- Не предлагай рефакторинг без прямого запроса.
...
| Маппинг | Ручной (методы `Copy*`/`Map*` в ViewModel) |
| Excel | EPPlus 8 |
| Иконки | FontAwesome5 |
| Логирование | Microsoft.Extensions.Logging (пакеты NLog 6 присутствуют, но в хост не подключены) |
...
- **Масштаб** — при создании участка/теплицы: `CanvasWidth = WidthMeters × scale` (по умолчанию `scale = 150` пикс/м).
- **EF Tools** — глобальный инструмент `dotnet-ef` сейчас версии `10.0.8` (проверка: `dotnet ef --version`), поэтому исходная проблема (старый инструмент под `net8.0` подтягивал `Microsoft.EntityFrameworkCore.Design 8.0.0`) больше не воспроизводится. На всякий случай **оба** проекта — `BonfireDB` и `Bonfire` — по-прежнему явно ссылаются на `Microsoft.EntityFrameworkCore.Design 10.0.8` с `PrivateAssets=all` (стандартная практика; гарантирует попадание пакета в `deps.json` стартового проекта). Миграции применяются автоматически при старте через `db.Database.MigrateAsync()`. Для инструментов добавлена `DbBonfireDesignTimeFactory`.

## Работа с ветками

- **Никогда не коммитить в `master` напрямую.** Вся работа ведётся в feature-ветках.
- Новая задача — новая ветка: `git switch -c feature/<name>`.
- Агенты запускаются с `isolation: "worktree"` — изменения изолированы в `.claude/worktrees/`.
- Мёрж в `master` только через явный запрос пользователя.
...
- Маппинг entity ↔ DTO выполняется вручную (методы `Copy*`/`Map*` в ViewModel). AutoMapper в проекте не используется.
