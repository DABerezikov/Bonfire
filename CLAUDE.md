...
- **EF Tools** — `dotnet ef database update` может падать из-за бинарной несовместимости глобального инструмента. Миграции всё равно применяются при старте приложения через `db.Database.MigrateAsync()`. Для инструментов добавлена `DbBonfireDesignTimeFactory`.
